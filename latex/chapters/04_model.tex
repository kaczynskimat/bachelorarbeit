...
Let $P_t$ be the set of all tuples available to an evaluating entity (such as gateway at Fog Layer or the Central Entity) at timestamp $t$. A consumption value $v$ is eligible for publication if and only if it satisfies the $z$-anonymity threshold:
\begin{equation}
    Count(v, P_t) \geq z
\end{equation}
