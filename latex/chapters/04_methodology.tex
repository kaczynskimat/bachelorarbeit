...
To evaluate the impact of preprocessing on privacy, we model a hierarchical \ac{IoT} architecture consisting of three distinct layers: the \textbf{Edge} (Smart Meters), the \textbf{Fog} (Gateways), and the \textbf{Central Entity} (Cloud). This model simulates the flow of energy consumption data from the household to the utility provider.
...
% I am not sure if I should explain that here as well, since I'm explaining that in the 'Layer 2: The Fog (Gateways)' section.
...
\begin{enumerate}
    \item \textbf{Data Volume:} It directly reduces the total row count of the transmitted dataset by a factor of $n$. This leads to significant \textit{Bandwidth Savings} (as defined in Section 4.5), which is a critical performance objective in resource-constrained \ac{IoT} environments.
